...
class Booking {
    ...
    constructor(bookingId:number, passenger:Passenger, flight:Flight, numberOfTickets : number, baggageWeight : number){
    ...
    private calculateTotalCost(baggageWeight:number):number {
...
class AirlineManager {
    private flightRepo: GenericRespository<Flight>;
    private passengerRepo : GenericRespository<Passenger>;
    private bookingRepo: GenericRespository<Booking>;

    private passengerIdCounter : number  = 0;
    private bookingIdCounter : number = 0;

    constructor(){
        this.flightRepo = new GenericRespository<Flight>();
        this.passengerRepo = new GenericRespository<Passenger>();
        this.bookingRepo = new GenericRespository<Booking>();
    }

    addFlight(flight:Flight): void {
        const existingFlight = this.flightRepo.find(f => (f as Flight).flightNumber === flight.flightNumber);
        if (existingFlight){
    ...
    createBooking(passengerId:number, flightNumber:string, numberOfTickets:number, baggageWeight:number): Booking | null {
        const passenger = this.passengerRepo.find(p => (p as Passenger).passengerId === passengerId);
        const flight = this.flightRepo.find(f => (f as Flight).flightNumber === flightNumber);
    ...
    cancelBooking (bookingId:number): void {
        const booking = this.bookingRepo.find(b => (b as Booking).bookingId === bookingId);
        if (!booking){
            console.error("Khong tim thay booking tuong ung");
            return;
        }
        const flight = (booking as Booking).flight;
        flight.bookedSeats -= (booking as Booking).numberOfTickets;
    ...
    const availableFlights = this.flightRepo.getAll().filter(
      f => !f.isFull() && f.origin.toLowerCase() === origin.toLowerCase() && f.destination.toLowerCase() === destination.toLowerCase());
    ...
    availableFlights.forEach(f => {
      ...
      console.log(
        `Chuyến bay: ${f.flightNumber} - ${flightType} | ${f.origin} -> ${f.destination} | Giờ khởi hành: ${f.departureTime.toLocaleString()} | Ghế trống: ${f.capacity - f.bookedSeats}`);
    });
  }

  listBookingsByPassenger(passengerId: number): void {
    const bookings = this.bookingRepo.getAll().filter(b => (b as Booking).passenger.passengerId === passengerId);
    if (bookings.length === 0) {
      console.log("Hành khách chưa có booking nào!");
      return;
    }

    bookings.forEach(b => console.log((b as Booking).getBookingDetails()));
  }

  calculateTotalRevenue(): number {
    const total = this.bookingRepo.getAll().reduce((sum, booking) => sum + (booking as Booking).totalCost, 0);
  ...
  getFlightPassengerList(flightNumber: string): void {
    const bookings = this.bookingRepo.getAll().filter(b => (b as Booking).flight.flightNumber === flightNumber);
    if (bookings.length === 0) {
      console.log("Chuyến bay chưa có hành khách!");
      return;
    }

    const passengers = bookings.map(b => (b as Booking).passenger.getDetails());
